...
void InferenceSession::ConstructorCommon(const SessionOptions& session_options,
  ...
  thread_pool_ = concurrency::CreateThreadPool("intra_op_thread_pool", session_options_.intra_op_num_threads);

  inter_op_thread_pool_ =
      session_options_.execution_mode == ExecutionMode::ORT_PARALLEL
          ? concurrency::CreateThreadPool("inter_op_thread_pool", session_options_.inter_op_num_threads)
          : nullptr;
